...
module.exports.run = async (bot,message,args,con) => {
  ...
  //     welcomemessage: "%member% joined the FALLEGA party!",
  //     leavemessage: "%member% left the FALLEGA party!",
  //     wlchannel: "",
  //     wlenable: "false",
  //     logs: "",
  //     logsenable: "false",
  ...
  //HERE STARTS WELCOME MESSAGE
   if(args[0] == "welcome-message"){
     ...
     con.query(`SELECT * FROM config WHERE id = '${message.guild.id}'`, (err, rows) => {
       ...
       if(rows == "") {
         sql = `INSERT INTO config (id, wm, lm, wl, wle, logs, logse) VALUES ('${message.guild.id}', '%member% joined the FALLEGA party!', '%member% left the FALLEGA party!', 'in-out', 'false', 'logs', 'false')`
       } else {
         let wm = rows[0].wm;
         let lm = rows[0].lm;
         let wl = rows[0].wl;
