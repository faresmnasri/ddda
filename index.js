...
bot.on("guildMemberAdd", async member => {
  ...
  //     welcomemessage: "%member% joined the FALLEGA_BOT party!",
  //     leavemessage: "%member% left the FALLEGA_BOT party!",
  //     wlchannel: "",
  //     wlenable: "false"
  //   };
  ...
  con.query(`SELECT * FROM config WHERE id = '${member.guild.id}'`, (err, rows) => {
    ...
    if(rows == "") {
      sql = `INSERT INTO config (id, wm, lm, wl, wle, logs, logse) VALUES ('${member.guild.id}', '%member% joined the FALLEGA_BOT party!', '%member% left the FALLEGA_BOT party!', 'in-out', 'false', 'logs', 'false')`
            con.query(sql);
    } else {
      let wm = rows[0].wm;
      let lm = rows[0].lm;
